...
export default function PortfolioPage() {
  ...
  // Charger les données de tous les portfolios (une seule fois)
  useEffect(() => {
  ...
  const handleUpdatePortfolio = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const portfolio = portfolios.find(p => p.id === editingPortfolio);
      if (portfolio) {
        const holdings = await portfoliosApi.getPortfolioHoldings(portfolio.id);
        const invested = holdings.reduce((sum, h) => sum + (h.investedAmount || 0), 0);
        const value = holdings.reduce((sum, h) => sum + (h.currentValue || 0), 0);
            ...
            <div className="mb-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h1 className={`text-2xl md:text-3xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                    {language === 'fr' ? 'Mes Portfolios' : 'My Portfolios'}
                  </h1>
                  <p className={`text-sm md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    {portfolios.length} {language === 'fr' ? 'portfolio' : 'portfolio'}{portfolios.length > 1 ? 's' : ''} • {globalStats.totalHoldings} {language === 'fr' ? 'positions' : 'positions'}
                  </p>
                </div>
                <Button 
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className={`rounded-xl p-5 ${isDarkMode ? 'bg-gray-800' : 'bg-white border border-gray-200'} shadow-sm`}>
                  ...
                  <div className={`text-xl md:text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                    {formatCurrency(globalStats.totalInvested)}
                  </div>
                </div>
                <div className={`rounded-xl p-5 ${isDarkMode ? 'bg-gray-800' : 'bg-white border border-gray-200'} shadow-sm`}>
                  <div className={`text-xs md:text-sm mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  ...
                  <div className={`text-xl md:text-2xl font-bold ${
                    globalStats.totalPNL >= 0 ? 'text-green-500' : 'text-red-500'
                  }`}>
                ...
                <div className={`rounded-xl p-5 ${isDarkMode ? 'bg-gray-800' : 'bg-white border border-gray-200'} shadow-sm`}>
                  ...
                  <div className={`text-xl md:text-2xl font-bold ${
                    globalStats.totalPNLPercentage >= 0 ? 'text-green-500' : 'text-red-500'
                  }`}>
                    {formatPercentage(globalStats.totalPNLPercentage)}
                  </div>
                </div>
                      ...
                      {data.holdings.length > 0 && (
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <button
                              onClick={() => router.push(`/portfolio?portfolioId=${portfolio.id}`)}
