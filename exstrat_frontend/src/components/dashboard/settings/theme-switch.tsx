...
export function ThemeSwitch(): React.JSX.Element {
	...
	const handleChange = React.useCallback(
		async (event: React.ChangeEvent<HTMLInputElement>) => {
			const newScheme = event.target.value as Mode;
			setMode(newScheme);

			const updatedSettings = { ...settings, theme: newScheme };
			await setPersistedSettings(updatedSettings);

			// Refresh the router to apply the new settings
			router.refresh();
		},
		[settings, setMode, router]
	);
				...
				<Card variant="outlined">
					...
					>
						{[
							...
							{ title: "System", description: "Adapts to your device's theme", value: "system", icon: DevicesIcon },
						].map((option) => (
							<FormControlLabel
								control={<Radio />}
								key={option.value}
