...
import { updateForecast, getTheoreticalStrategies, getPortfolioHoldings, getForecastById } from "@/lib/portfolios-api";
...
export function EditForecastModal({ onClose, onSuccess, open, forecast }: EditForecastModalProps): React.JSX.Element {
	...
	// Load forecast data when modal opens
	React.useEffect(() => {
		if (open && forecast) {
			loadForecastData();
			refreshPortfolios();
	...
	}, [open, forecast, refreshPortfolios]);
	...
	const loadForecastData = async () => {
		...
		try {
			setLoading(true);
			// Load full forecast data
			const forecastData = await getForecastById(forecast.id);
			setForecastName(forecastData.name || "");
			const portfolioId = forecastData.portfolioId || "";
			setSelectedPortfolioId(portfolioId);
			// Set applied strategies from forecast
			setAppliedStrategies(forecastData.appliedStrategies || {});
			// Load holdings for the portfolio
			if (portfolioId && portfolioId !== "virtual") {
				await loadHoldingsForPortfolio(portfolioId, forecastData.appliedStrategies || {});
			}
		} catch (error) {
	...
	const loadHoldingsForPortfolio = async (portfolioId: string, existingStrategies: Record<string, string>) => {
		if (!portfolioId || portfolioId === "virtual") return;

		try {
			const holdingsData = await getPortfolioHoldings(portfolioId);
			setHoldings(holdingsData || []);
			// Initialize applied strategies with existing values from forecast or 'none'
			const initialStrategies: Record<string, string> = {};
			holdingsData.forEach((holding: any) => {
				initialStrategies[holding.id] = existingStrategies[holding.id] || "none";
			});
	...
	const loadHoldings = async () => {
		...
		try {
			setLoading(true);
			await loadHoldingsForPortfolio(selectedPortfolioId, appliedStrategies);
		} catch (error) {
			console.error("Error loading holdings:", error);
			setHoldings([]);
		} finally {
	...
	const handleSubmit = async () => {
		...
		if (errors.forecastName || errors.strategies) {
			return;
		}
		...
		try {
			...
			const forecastData: Partial<CreateForecastDto> = {
		...
		<Dialog fullWidth maxWidth="lg" onClose={handleClose} open={open}>
			<DialogTitle>
				<Stack direction="row" spacing={2} sx={{ alignItems: "center", justifyContent: "space-between" }}>
					<Typography variant="h6">Edit Forecast</Typography>
			...
			<DialogContent dividers>
				<Stack spacing={4}>
					{/* Error Alert */}
					{(fieldErrors.forecastName || fieldErrors.strategies) && (
					...
					<Stack spacing={2}>
						...
						<Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
							<FormControl fullWidth>
								...
								<FormHelperText>Wallet cannot be changed</FormHelperText>
							</FormControl>
							<TextField
								fullWidth
								label="Forecast name"
			...
			<DialogActions>
				...
				<Button
					...
					variant="contained"
				>
					{isSubmitting ? "Updating..." : "Update Forecast"}
				</Button>
			</DialogActions>
		</Dialog>
	);
